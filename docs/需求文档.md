...
| `/v1/admin/connector-registry/refresh` | 扫描 `source-repo/` 并刷新 connector registry |
...
6. pricing rules 达标。
7. mock provider/account/mapping 仅作为开发自测可选证据；清空假数据后的生产验证环境不依赖它。
...
- [ ] `GET /v1/runtime` 正常。
- [ ] `GET /v1/models` 返回模型。
- [ ] provider catalog / provider-model mappings 覆盖默认逻辑模型。
- [ ] 清空假数据后 readiness `core_ready=true` 不依赖 mock provider/account/mapping。
- [ ] 如需开发稳定性证据，单独运行 mock/stability self-test。
