...
def main() -> None:
    ...
    try:
      with TestClient(app) as client:
        ...
        production_unblock = assert_ok(client.get("/v1/admin/proxy-kernels/production-unblock-package", headers=headers))
        assert production_unblock["object"] == "media2api.proxy_kernel.production_unblock_package", production_unblock
        assert production_unblock["summary"]["complete_coverage_candidates"] >= 1 and production_unblock["recommended_provider_ids"], production_unblock
        assert production_unblock["recommended_provider_ids"][0] == "gemini_cli_oauth", production_unblock
        ...
        for stability_dom in ["/v1/admin/stability/acceptance-evidence", "/v1/admin/stability/acceptance-suite"]:
            assert stability_dom in admin_page.text, stability_dom
        ...
        for account_material_dom in ["kernel-account-materials", "kernel-account-materials-matrix", "kernel-account-connection-package", "kernel-production-unblock-package", "kernel-production-unlock-wizard", "kernel-credential-intake-load", "kernel-credential-intake-panel", "kernel-bulk-account-materials-json", "kernel-bulk-account-preflight", "kernel-bulk-account-import", "kernel-bulk-account-import-accept", "loadKernelCredentialIntakeSheet", "submitKernelBulkAccountMaterials", "renderKernelCredentialIntakeSheet", "生产解锁向导", "批量 dry-run", "导入并验收", "/credential-intake-sheet", "kernel-account-material-panel", "kernel-account-credential", "kernel-account-resource-profile", "kernel-account-preflight", "kernel-account-import", "kernel-account-runtime-sync", "kernel-account-runtime-sync-status", "submitKernelAccountMaterials", "syncKernelRuntimeCredentials", "renderKernelAccountMaterialsPanel", "loadKernelAccountMaterials", "renderKernelAccountMaterialsMatrix", "loadKernelAccountMaterialsMatrix", "renderKernelAccountConnectionPackage", "loadKernelAccountConnectionPackage", "renderProductionUnblockPackage", "loadProductionUnblockPackage", "resource_profile_json_template", "fields_by_destination", "/account-materials", "/account-materials-matrix", "/account-connection-package", "/production-unblock-package", "/account-materials-bulk", "/runtime-credentials/sync"]:
        ...
        stability_evidence = assert_ok(client.get("/v1/admin/stability/acceptance-evidence", headers=headers))
        assert stability_evidence["object"] == "media2api.stability_acceptance_evidence" and "ok" in stability_evidence, stability_evidence
        stability_suite = assert_ok(client.post("/v1/admin/stability/acceptance-suite", headers=headers, json={"iterations": 3, "cleanup": True, "persist_evidence": False}))
        assert stability_suite["object"] == "media2api.stability_acceptance_evidence" and stability_suite["status"] == "action_required", stability_suite
        assert stability_suite["rows"]["AC-S-001"]["ok"] is False and stability_suite["rows"]["AC-S-001"]["evidence"]["iterations_required"] == 1000, stability_suite
        for stability_row_id in ["AC-S-002", "AC-S-003", "AC-S-004", "AC-S-005"]:
            assert stability_suite["rows"][stability_row_id]["ok"] is True, stability_suite
        assert stability_suite["cleanup"]["enabled"] is True and stability_suite["cleanup"]["job_count"] >= 1, stability_suite
