...
PLATFORM_INPUT_REQUIREMENTS: dict[str, dict[str, Any]] = {
    ...
    "gemini": {
        ...
        "user_inputs": [
            {"name": "gemini_credentials", "label": "GEMINI_CREDENTIALS JSON", "required": True, "any_of_group": "gemini_oauth_material", "auth_method": "agent_provider_credential", "store_as": "encrypted_secret_or_agent_ref", "evidence": "gzzhongqi__geminicli2api/.env.example: copy the GEMINI_CREDENTIALS JSON string directly; it overrides file-based credentials"},
        ...
        "user_actions": [
            "从本机 Gemini CLI/Antigravity 授权结果读取 %USERPROFILE%\\.gemini\\oauth_creds.json 或 ~/.gemini/oauth_creds.json。",
            "或复制 geminicli2api .env 中的 GEMINI_CREDENTIALS JSON；若使用 GOOGLE_APPLICATION_CREDENTIALS=oauth_creds.json，则复制该文件内容。",
            "如果使用 AIClient2API，则复制 --gemini-oauth-creds-file 指向的文件内容，或把 --gemini-oauth-creds-base64 的值填入 gemini_oauth_creds_base64。",
    ...
    "grok": {
        ...
        "user_inputs": [
            {"name": "grok_cookie_or_session", "label": "Grok Web/Build cookie 或 session", "required": True, "auth_method": "cookie_secret", "store_as": "encrypted_secret", "evidence": "chenyme__grok2api/config.defaults.toml: proxy.clearance manual mode uses cf_cookies plus matching user_agent copied from the logged-in browser session"},
            {"name": "user_agent", "label": "同一次浏览器请求的 User-Agent", "required": False, "auth_method": "cookie_secret", "store_as": "encrypted_secret", "evidence": "chenyme__grok2api/config.defaults.toml: proxy.clearance.user_agent must match the browser session used for cf_cookies"},
        ...
        "user_actions": [
            "在本人已登录的 grok.com/Grok Build 浏览器页面打开 DevTools -> Network，刷新页面并复制发往 grok.com/x.ai 请求的 Cookie header。",
            "从同一次请求的 Request Headers 复制 User-Agent；grok2api 要求 cf_cookies 与 user_agent 匹配。",
            "如果 Network 无 Cookie header，到 Application -> Cookies -> grok.com 复制 cookie，并拼成 name=value; name2=value2 格式。",
            "把 Cookie header 填入 grok_cookie_or_session，把 User-Agent 填入 user_agent，然后验收 Imagine 图片/视频输出。",
        ],
    },
    "jimeng": {
        "primary_resource_type": "agent_provider",
...
PROVIDER_GUIDE_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "gemini": {
        ...
        "base_url_example": "",
        "steps": [
            "从 Gemini CLI/Antigravity 授权文件获取 OAuth JSON：Windows 通常是 %USERPROFILE%\\.gemini\\oauth_creds.json，Linux/macOS 通常是 ~/.gemini/oauth_creds.json。",
            "也可以复制 geminicli2api 的 GEMINI_CREDENTIALS JSON，或 GOOGLE_APPLICATION_CREDENTIALS 指向的 oauth_creds.json 文件内容。",
            "使用 AIClient2API 时，复制 --gemini-oauth-creds-file 指向文件内容，或把 --gemini-oauth-creds-base64 的值作为 gemini_oauth_creds_base64 提交。",
            "只有使用外部 agent wrapper 服务时才填写 runtime/base_url。",
